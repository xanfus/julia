...
function _unchecked_cast(::Type{Int64}, x::BigFloat, r::MPFRRoundingMode)
    r = ccall((:__gmpfr_mpfr_get_sj,:libmpfr), Cintmax_t, (Ref{BigFloat}, MPFRRoundingMode), x, r)
end

function _unchecked_cast(::Type{UInt64}, x::BigFloat, r::MPFRRoundingMode)
    r = ccall((:__gmpfr_mpfr_get_uj,:libmpfr), Cuintmax_t, (Ref{BigFloat}, MPFRRoundingMode), x, r)
end
...
function round(::Type{T}, x::BigFloat, r::Union{RoundingMode, MPFRRoundingMode}) where T<:Union{Signed, Unsigned}
    clear_flags()
    r = _unchecked_cast(T, x, r)
    if had_range_exception() || !(typemin(T) <= r <= typemax(T))
        throw(InexactError(:round, T, x))
    end
    return unsafe_trunc(T, r)
end
